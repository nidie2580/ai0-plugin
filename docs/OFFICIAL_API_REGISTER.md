...
当前「平台用户名 + 绑定邮箱 == <QQ>@qq.com」的校验偏弱：QQ 图片转文字可能识别偏差，且邮箱格式本身可被构造。下一版协议建议二选一：

1. **邮箱验证码**：插件关联请求后，平台向绑定邮箱发验证码，管理员在网页后台回填，平台校验通过才算关联成功。
2. **网页授权绑定**：管理员在平台网页登录态下点「授权绑定本实例」，平台把 instance_id 与账号写进绑定表，插件只读结果。

两端需同时排期，协议定稿前本插件维持现有两段式校验（QQ 优先匹配 + 手填邮箱兜底）。
