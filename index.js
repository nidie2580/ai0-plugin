...
setTimeout(() => {
  try {
    ...
    logger.info(`[ai0-plugin] 主人列表：框架(${s.framework.length})=[${s.framework.join(',') || '-'}] + 插件(${s.plugin.length})=[${s.plugin.join(',') || '-'}] → 合并 ${all.length} ...`)
    if (!all.length) {
      logger.warn(`[ai0-plugin] ⚠️ 未检测到任何主人！可在 Yunzai 全局 config/matcher.master 或 插件 config.yaml 的 permissions.masters 中配置。任何人都可以发送 #ai[...]`)
    }
  } catch (e) {
