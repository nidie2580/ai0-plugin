...
if (typeof segment === 'undefined' || !segment) {
  if (typeof logger !== 'undefined') logger.warn('[ai0-plugin] global.segment 未注入，图片发送功能可能不可用')
  global.segment = {}
}
...
setTimeout(() => {
  try {
    ...
    if (typeof logger !== 'undefined') logger.warn(`[ai0-plugin] 读取主人列表失败：${e.message}`)
  }
}, 600)
  ...
  try {
    ...
    if (autoStart) {
      ...
      try {
        ...
        if (typeof logger !== 'undefined') logger.warn(`[ai0-plugin] 网页后台启动失败（端口占用？）：${err.message}`)
      }
    }
  } catch (e) {
    if (typeof logger !== 'undefined') logger.warn(`[ai0-plugin] 初始化网页后台出错：${e.message}`)
  }
}, 800)
...
if (fs.existsSync(appsDir)) {
  for (const file of fs.readdirSync(appsDir).filter(f => f.endsWith('.js'))) {
    if (!ALLOWED_APPS.has(file)) {
      if (typeof logger !== 'undefined') logger.warn(`[ai0-plugin] 跳过未授权模块 ${file}（不在白名单中）`)
      continue
    }
    try {
      ...
      if (typeof logger !== 'undefined') logger.error(`[ai0-plugin] 加载模块 ${file} 失败: ${err.message}`)
    }
  }
}
