...
export function buildMultiChatRequest({ reqHistory, archiveReplies, modelKey, modelDisplay, multiChatEnabled }) {
  ...
  if (lastIdx >= 0 && next[lastIdx].role === 'user') {
    ...
    // 追加一个 text 部分承载互聊发言，保留原有 image_url。
    const c = next[lastIdx].content
    if (Array.isArray(c)) {
      next[lastIdx] = { ...next[lastIdx], content: [...c, { type: 'text', text: others }] }
    } else {
      next[lastIdx] = { ...next[lastIdx], content: `${String(c || '')}\n\n${others}` }
    }
  }
  return next
}
