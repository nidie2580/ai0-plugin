...
import { GroupBroadcast } from '../src/broadcast.js'
...
const mockBot = {
  getGroups: async () => [
    { group_id: '1', group_name: '测试群1' },
    { group_id: '2', group_name: '测试群2' }
  ],
  getGroup: async (groupId) => {
    const groups = await mockBot.getGroups()
...
async function runTests() {
  ...
  console.log('\n4. 测试群体广播功能')
  try {
    const broadcast = new GroupBroadcast(mockBot)
    
    // 测试获取群聊列表
    const groups = await broadcast.getGroups()
    logTest('获取群聊列表', true, `找到 ${groups.length} 个群聊`)
    passedCount++
    
    // 测试群聊统计
    const stats = await broadcast.getGroupStats()
    logTest('获取群聊统计', true, `总群聊数: ${stats.totalGroups}`)
    passedCount++
    
  } catch (error) {
    logTest('群体广播功能', false, error.message)
    failedCount += 2
  }
