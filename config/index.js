...
agent:
  # 是否启用 Agent 能力。默认开，主人会话可获得加 curl/wget 的网络访问能力；
  # 非主人一律不注入上下文（masterOnly 门控），因此对非主人无暴露
  enabled: true
  # 仅允许主人（permissions.masters）触发。命令执行权限较高，默认仅主人
  masterOnly: true
  # 单次任务最大执行轮数（AI 出命令→执行→回传 为一轮，上限 20）。
  # 网页后台可配置，默认 5（比旧值 8 更保守）：低配额 API（如 RPM≤3）建议 2~3；高配额可 5~8
  maxRounds: 5
  # 单条命令超时（毫秒）
